...
const STATUS_COLORS = { pending:'badge-pending', preparing:'badge-preparing', ready:'badge-ready', paid:'badge-paid', cancelled:'badge-cancelled', served:'badge-served' }
...
export default function Orders() {
  ...
  const handleKitchen = async (id, ks) => {
    try {
      await updateKitchenStatus(id, ks)
      toast.success(`Kitchen: ${ks}`)
    } catch { toast.error('Failed') }
  }
  ...
  return (
    <div className="orders-page fade-in">
      <div className="page-header">
        <div><div className="page-title">Orders</div><div className="page-subtitle">Manage all restaurant orders</div></div>
      </div>
      ...
      <div className="orders-layout">
        ...
        <div className="card" style={{ flex: 1, overflow: 'auto' }}>
          <table className="data-table">
            <thead>
              <tr><th>#</th><th>Type</th><th>Table</th><th>Items</th><th>Total</th><th>Status</th><th>Kitchen</th><th>Time</th><th>Actions</th></tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={9} style={{ textAlign:'center', color:'var(--text-muted)', padding: 40 }}>No orders found</td></tr>
              ) : filtered.map(o => (
                <tr key={o.id} style={{ cursor: 'pointer' }} onClick={() => setSelected(o.id)}>
                  <td style={{ fontWeight: 700, color: 'var(--text-primary)' }}>#{String(o.orderNumber||o.id.slice(0,4)).padStart(4,'0')}</td>
                  ...
                  <td onClick={e=>e.stopPropagation()}>
                    <div style={{ display:'flex', gap:6 }}>
                      {o.status === 'pending' && (
                        <button className="btn-outline" style={{ padding:'4px 10px', fontSize:11 }} onClick={() => { handleStatus(o.id,'preparing'); handleKitchen(o.id,'preparing') }}>Prepare</button>
                      )}
                      {o.status === 'preparing' && (
                        <button className="btn-outline" style={{ padding:'4px 10px', fontSize:11 }} onClick={() => { handleStatus(o.id,'ready'); handleKitchen(o.id,'ready') }}>Ready</button>
                      )}
                      {o.status === 'ready' && (
                        <button className="btn-outline" style={{ padding:'4px 10px', fontSize:11 }} onClick={() => handleStatus(o.id,'served')}>Served</button>
                      )}
                      {['pending','preparing','ready','served'].includes(o.status) && (
                        <button className="btn-primary" style={{ padding:'4px 10px', fontSize:11 }} onClick={() => setSelected(o.id)}>Pay</button>
                      )}
                    </div>
                  </td>
                </tr>
        ...
        {detail && (
          <div className="card orders-detail fade-in">
            ...
            <div className="orders-detail-info">
              <div><span>Customer</span><span>{detail.customerName||'Walk-in'}</span></div>
            ...
            {detail.paymentStatus !== 'paid' && (
              <div style={{ marginTop:16, display:'flex', flexDirection:'column', gap:10 }}>
                ...
                <button className="btn-primary" style={{ width:'100%', padding:12 }} onClick={handlePay} disabled={paying || (payMethod==='cash' && Number(amountPaid)<detail.total)}>
            ...
            {detail.paymentStatus === 'paid' && (
              <div style={{ marginTop:16, padding:'10px 14px', background:'rgba(45,198,83,0.1)', borderRadius:8, color:'#2dc653', fontWeight:600, textAlign:'center' }}>
